...
class Corpus(object):
    ...
    def maximization_step(self, number_of_topics):
        ...
        print("M step:")
        # update P(w | z)
        for topic_idx in tqdm(range(number_of_topics)):
            for word_idx in range(self.vocabulary_size):
                prob = self.term_doc_matrix[:, word_idx] * self.topic_prob[:, topic_idx, word_idx]
                self.topic_word_prob[topic_idx, word_idx] = prob.sum()
            self.topic_word_prob[topic_idx, :] /= self.topic_word_prob[topic_idx, :].sum()
            
        # update P(z | d)
        for document_idx in tqdm(range(self.number_of_documents)):
            for topic_idx in range(number_of_topics):
                prob = self.term_doc_matrix[document_idx, :] * self.topic_prob[document_idx, topic_idx, :]
                self.document_topic_prob[document_idx, topic_idx] = prob.sum()
            self.document_topic_prob[document_idx, :] /= self.document_topic_prob[document_idx, :].sum()
    ...
    def plsa(self, number_of_topics, max_iter, epsilon):
        ...
        # P(z | d, w)
        self.topic_prob = np.zeros([self.number_of_documents, number_of_topics, self.vocabulary_size], dtype=np.float32)
...
def main():
    documents_path = 'C:/Users/bhara/OneDrive - scu.edu/Code Projects/NLP/P3/DBLP_1000.txt'
